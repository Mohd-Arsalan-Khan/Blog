...
function DashProfile() {
    ...
    const handleChange = (e) =>{
        const updateData = ({...formData, [e.target.id] : e.target.value})
        setFormData(updateData)
        dispatch(updateSuccess({...currentUser,[e.target.id]: e.target.value}))
    }   
    ...
    const handleSubmit = async (e) =>{
        ...
        if (Object.keys(formData).length === 0) {
            setUpdateUserError("No changed made")
            return;
        }
        if (imageFileUploading) {
            setUpdateUserError("Please wait to upload the image")
            return;
        }
        try {
            ...
            if (!res.ok) {
                ...
                setUpdateUserSuccess("User profile update sucessfully")
